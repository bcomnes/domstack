...
for (const width of [1500, 390]) {
  test(`reading position updates the URL and sidebar without navigating at ${width}px`, async ({ page, siteURL }) => {
    ...
    const scrollToHeading = async locator => {
      return locator.evaluate(heading => {
        ...
        scrollTo(0, scrollY + heading.getBoundingClientRect().top - offset + 2)
        return scrollY
      })
    }
