...
- They should be included in the build output manifest so PWA tooling can reason about them.
...
- Do not make watch mode write `domstack-output-manifest.json`.
...
5. `buildOutputManifest()` includes the service worker entry like any other emitted output.

The service worker itself can fetch `/domstack-output-manifest.json` at runtime with `cache:
...
Regenerate `lib/build-output-manifest/schema.json` with `npm run build:schema`.
...
- no `results.outputManifest`
- no `domstack-output-manifest.json`
...
- output manifest includes `kind: 'service-worker'`.
- watch mode builds the service worker but does not write `domstack-output-manifest.json`.
