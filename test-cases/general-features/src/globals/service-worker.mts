import { CACHE_PREFIX, DOMSTACK_MANIFEST_URL, DOMSTACK_MANIFEST_VERSION, loadManifest } from '../libs/service-worker-helper.js'
...
async function precache () {
  ...
  const cache = await caches.open(CACHE_PREFIX + DOMSTACK_MANIFEST_VERSION)
...
async function cleanup () {
  const current = CACHE_PREFIX + DOMSTACK_MANIFEST_VERSION
