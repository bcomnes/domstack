...
export default async function domstackManifestSettings () {
  return {
    filename: 'settings/domstack-manifest.json',
    exclude: ['settings/**'],
    includeEntry (entry) {
...
`)
      ...
      await stat(path.join(settingsDest, 'settings/domstack-manifest.json'))
      await assert.rejects(
        () => stat(path.join(settingsDest, 'domstack-manifest.json')),
        'domstack-manifest.settings.js custom filename replaces the default manifest path'
      )
      assert.ok(
        settingsServiceWorkerContent.includes('/settings/domstack-manifest.json'),
        'service worker define receives domstack-manifest.settings.js custom filename'
      )
      assert.ok(
