...
runs a manifest-enabled build so the PWA path works there. It also serves without live-reload HTML
injection so fetched files match the domstack manifest revisions. Wait until the UI says `Offline
cache current` before testing an offline refresh. Use `npm run watch` for development without a
sticky service worker cache. To clear all example workers and caches:
