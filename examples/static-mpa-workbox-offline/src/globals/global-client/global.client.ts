...
const config = {
  ...
  manifestEnabled: process.env.DOMSTACK_MANIFEST_ENABLED === 'true',
  offlineRecheckIntervalMs,
  offlineStorageKey,
  onlineCheckTimeoutMs,
  serviceWorkerScope: process.env.DOMSTACK_SERVICE_WORKER_SCOPE,
  serviceWorkerUrl: process.env.DOMSTACK_SERVICE_WORKER_URL,
}

const status = createStatusBanner()

initializeConnectionStatus(status, config)
initializeServiceWorkerEventMessages(status)

const { serviceWorkerScope, serviceWorkerUrl } = config

if (serviceWorkerUrl && serviceWorkerScope && 'serviceWorker' in navigator) {
  if (shouldResetServiceWorker()) {
    try {
      await resetServiceWorkerFromWindow(status, config)
    } catch (error) {
      console.error('Service worker reset failed', error)
    }
  } else if (!config.manifestEnabled) {
    try {
      await disableServiceWorkerForWatchMode(status, config)
    } catch (error) {
