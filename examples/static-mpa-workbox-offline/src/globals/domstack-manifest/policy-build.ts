...
function buildWorkboxPolicy (
  ...
  const precacheManifest: WorkboxPrecacheEntry[] = []
  const runtimeUrls: string[] = []
  const networkOnlyUrls: string[] = []

  for (const entry of manifest.entries) {
    const runtimeStrategy = runtimeStrategyFor(entry, routes)
    if (shouldPrecache(entry, runtimeStrategy)) {
      precacheManifest.push(toWorkboxPrecacheEntry(entry))
    } else if (runtimeStrategy === 'runtime') {
      runtimeUrls.push(entry.url)
    } else if (runtimeStrategy === 'network-only') {
      networkOnlyUrls.push(entry.url)
    }
  }
