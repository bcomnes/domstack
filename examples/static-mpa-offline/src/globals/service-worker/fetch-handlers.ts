...
export function handleFetchEvent (config: ActiveServiceWorkerConfig, event: FetchEvent): void {
  ...
  if (request.mode === 'navigate') {
    event.respondWith(handleNavigation(config, request, event.preloadResponse))
    return
  }
...
/** Apply route policy to a non-navigation request before falling back to static cache-first handling. */
...
/** Match a navigation against canonical static-MPA URL variants in the precache. */
...
/** Ignore local development-server requests that should never be cached. */
...
/** Return whether the request inherits an explicit runtime-cache route policy. */
...
/** Resolve policy directly for an output, or inherit policy from its same-origin referrer page. */
