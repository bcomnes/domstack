...
`siteData` remains the discovery result from `identifyPages()`, both in a pages factory and in the public `results.siteData` returned by a build. Its `siteData.pages` array contains only source-backed pages. Generated pages are created later inside the page worker and are not added to this public discovery list.
