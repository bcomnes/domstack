...
/**
 * @typedef {Awaited<ReturnType<typeof copy>> & { outputs: BuildOutputRecord[] }} StaticBuilderReport
 */
...
export async function buildStatic (src, dest, _siteData, opts) {
  ...
  try {
    const report = /** @type {StaticBuilderReport} */ (await copy(getCopyGlob(src), dest, ...(opts?.ignore ? [{ ignore: opts.ignore }] : [])))
    report.outputs = createCopiedOutputRecords({
      src,
      dest,
      report,
      kind: 'static',
