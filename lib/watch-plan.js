...
export function planWatchBatch (state, events) {
  ...
  const sourcePaths = new Set(pagePaths(state.siteData.pages.filter(page => !page.generated)))
  ...
  for (const event of events) {
    ...
    const changePlan = structural ? planWatchEvent(state, { ...event, type: 'change' }) : eventPlan
    ...
    const sourceEvent = event.name.endsWith('.md') ||
    ...
    for (const path of allInputs || selected === null ? sourcePaths : selected) {
