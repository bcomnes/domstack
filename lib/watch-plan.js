 ...
 * @typedef {PagePlan | {kind: 'skip', message: string} | {kind: 'full', message: string, warning?: string} | {kind: 'restart', message: string}} WatchPlan
 */
import { basename, dirname, relative } from 'node:path'
import { classifyFile, esbuildSettingsNames } from './file-conventions.js'
...
export function planWatchEvent (state, event) {
  ...
  if (convention?.change === 'full') {
    return {
      kind: 'full',
      message: `"${name}" changed, triggering full rebuild...`,
      ...(esbuildSettingsNames.includes(name)
        ? { warning: `"${name}" changed, but esbuild settings are cached in the DOMStack process. Stop and restart DOMStack to apply these changes; restarting esbuild alone does not reload the settings module.` }
        : {}),
    }
  }
