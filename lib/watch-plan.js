 ...
 * @property {boolean} [hasGlobalDataState] The watch session retains producer state.
...
export function planWatchEvent (state, event) {
  ...
  const markdownChanged = convention?.change === 'markdown'
  if (markdownChanged) {
 ...
 * Structural source events upsert mapped source consumers plus the event path
 * (including removed paths). Other structural events upsert all
 ...
 *
 * A reset supersedes the delta. Reasons are page-build-failed, then the first
 * applicable event reason, using this per-event priority:
 * unreliable-event, global-data-changed, global-vars-changed,
 * markdown-settings-changed, global-config-changed, or unknown-event.
 * Only non-module source texts (.md/.html) can use deltas.
 * Other inputs may also configure global vars or Markdown rendering.
 * Resets force full output rebuilds only when producer state is retained;
 * without retained state, legacy event routing still determines output scope.
 * Failed builds retain their recovery plan; the executor handles rediscovery.
...
export function planWatchBatch (state, events) {
  ...
  let resetReason = state.pageBuildFailed ? 'page-build-failed' : undefined
  ...
  for (const event of events) {
    ...
    const reason = inputResetReason(state, event)
    resetReason ??= reason
    plan = unionWatchPlans(plan, eventPlan)
    if (reason === 'unreliable-event') {
  ...
  } else if (state.hasGlobalDataState && resetReason) {
    plan = unionWatchPlans(plan, { kind: 'full', message: 'Producer inputs require a reset, triggering full rebuild...' })
  }
 ...
 * @param {WatchEvent} event
 */
function inputResetReason (state, { type, filepath, name, convention }) {
  ...
  const sourcePage = state.pageFileMap.get(filepath)
  const sourceText = name.endsWith('.md') || name.endsWith('.html')
  if (sourceText && (
    (sourcePage && !sourcePage.generated && sourcePage.pageFile.filepath === filepath) ||
    (type !== 'change' && (name.endsWith('.md') || convention?.change === 'page'))
  )) return
...
function unionWatchPlans (left, right) {
  ...
  return {
    ...
    pages: left.pages || right.pages
      ? [...new Map([...(left.pages ?? []), ...(right.pages ?? [])].map(page => [page.pageFile.filepath, page])).values()]
      : undefined,
    templates: left.templates || right.templates
      ? [...new Map([...(left.templates ?? []), ...(right.templates ?? [])].map(template => [template.templateFile.filepath, template])).values()]
      : undefined,
