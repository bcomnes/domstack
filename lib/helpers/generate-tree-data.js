 ...
 * @return {Partial<TreeNode>} A tree structure ready to print
 */
export function generateTreeData (cwd, src, dest, results) {
  ...
  const templateOutputsBySource = groupOutputsBySource(
    getBuildOutputs(results?.pageBuildResults).filter(output => output.kind === 'template')
  )
  ...
  const staticOutputs = getBuildOutputs(results?.staticResults).filter(output => output.kind === 'static')
  ...
  // clean-deep is a CommonJS package whose bundled declaration does not model
  // Node's default-import interop under our TypeScript module settings.
  const cleanTree = /** @type {(tree: TreeNode) => Partial<TreeNode>} */ (
    /** @type {unknown} */ (cleanDeep)
  )
  return cleanTree(treeStructure)
}

/**
 * @param {{ outputs?: DomstackManifestRecord[] } | null | undefined} result
 * @returns {DomstackManifestRecord[]}
 */
function getBuildOutputs (result) {
