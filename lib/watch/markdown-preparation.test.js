 ...
 * @import { WorkerBuildMessage } from '../build-pages/worker/protocol.js'
...
/** @param {TestContext} t @param {{ src: string }} site */
function probePreparationInput (t, site) {
  const postMessage = Worker.prototype.postMessage
  /** @this {Worker} @param {Parameters<Worker['postMessage']>} args */
  function recordInput (...args) {
    const message = /** @type {WorkerBuildMessage | null} */ (args[0])
    if (message?.type === 'build' && message.src === site.src) {
      const previous = message.opts.previousMarkdownPreparation
      // Record synchronously before dispatch so the worker's source reads follow.
      appendFileSync(join(site.src, '../preparation.jsonl'), JSON.stringify({
        kind: 'input',
        previous: previous == null
          ? null
          : [...previous].map(([filepath, entry]) => ({
              filepath: relative(site.src, filepath),
              sourceId: entry.sourceId,
              title: entry.prepared.vars['title'],
            })).sort((a, b) => a.sourceId.localeCompare(b.sourceId)),
        resetReason: message.opts.globalDataInputChanges?.resetReason ?? null,
      }) + '\n')
    }
    return postMessage.apply(this, args)
  }
  t.mock.method(Worker.prototype, 'postMessage', recordInput)
}
...
const preparationProbeLayout = `import fs from 'node:fs/promises'
  ...
  fs.readFile = async (...args) => {
    if (String(args[0]).endsWith('.md')) record({ kind: 'read', path: relative(src, String(args[0])) })
