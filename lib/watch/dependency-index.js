...
export class WatchDependencyIndex {
  ...
  async rebuild (siteData, { reuseAnalysis = false, isObserved = unobserved } = {}) {
    ...
    const { dependenciesFor, nextAnalysis } = createDependencyAnalysis(
      cwd,
      new Map(this.#analysisCache),
      filepath => this.#findDependencies(filepath),
      isObserved
    )
...
/**
 * Share dependency and observation promises within one rebuild, collecting candidates for retention.
 * @param {string} cwd
 * @param {Map<string, string[]>} previousAnalysis
 * @param {typeof find} findDependencies
 * @param {DependencyObservation} isObserved
 */
function createDependencyAnalysis (cwd, previousAnalysis, findDependencies, isObserved) {
  ...
  const dependenciesFor = filepath => {
    ...
    if (!dependencies) {
      ...
      if (cached) {
        ...
        dependencies = findDependencies(key).then(async deps => {
          const resolved = deps.map(dep => resolve(cwd, dep))
          if (isObserved !== unobserved && (await Promise.all([key, ...resolved].map(observed))).every(Boolean)) {
            nextAnalysis.set(key, resolved)
          }
          return resolved
        })
      }
  ...
  return { dependenciesFor, nextAnalysis }
}
