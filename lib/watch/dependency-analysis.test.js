...
test('fresh rebuilds bypass retained analysis and seed later opt-in reuse only with explicit observation', async t => {
  ...
  await index.rebuild(site, { isObserved: observed })
  assert.equal(count('global.data.js'), 5, 'another fresh rebuild reanalyzes despite the retained baseline')
  await index.rebuild(site, reuse)
  assert.equal(count('global.data.js'), 5, 'the fresh observed result remains eligible for reuse')
})

test('injected analyzers keep the index receiver and start without an extra async hop', async t => {
  const { site, define, file } = fixture(t)
  site.globalData = define('global.data.js')
  const findDependencies = t.mock.fn(async function () {
    return [relative(process.cwd(), file('helper.json').filepath)]
  })
  const index = new WatchDependencyIndex(pino({ level: 'silent' }), findDependencies)
  for (let pass = 1; pass <= 2; pass++) {
    const pending = index.rebuild(site)
    assert.equal(findDependencies.mock.callCount(), pass, 'direct calls start fresh analysis synchronously')
    await pending
    assert.equal(findDependencies.mock.calls[pass - 1]?.this, index)
    assert.deepEqual(findDependencies.mock.calls[pass - 1]?.arguments, [site.globalData.filepath])
    assert.deepEqual(index.snapshot().globalDataDepPaths, new Set([site.globalData.filepath, file('helper.json').filepath]))
  }
  await index.rebuild(site, reuse)
  await index.rebuild(site, reuse)
  assert.equal(findDependencies.mock.callCount(), 3)
  assert.equal(findDependencies.mock.calls[2]?.this, index)
})
...
test('distinct roots share observation promises for duplicate inputs, including failed observations', async t => {
  for (const outcome of ['observed', 'unobserved', 'thrown', 'rejected']) {
    ...
    const options = { reuseAnalysis: true, isObserved }
    const observationOrder = [site.globalData.filepath, file('helper.json').filepath, site.globalVars.filepath]
    for (let pass = 1; pass <= 2; pass++) {
      await index.rebuild(site, options)
      const analyses = outcome === 'observed' ? 1 : pass
      assert.deepEqual(['global.data.js', 'global.vars.js'].map(count), [analyses, analyses], outcome)
      assert.deepEqual(isObserved.mock.calls.map(call => call.arguments[0]),
        Array.from({ length: analyses }, () => observationOrder).flat(),
        'each input is observed once per pass, in discovery order')
...
test('reused analysis reconstructs current discovery objects and layout selections without changing old snapshots', async t => {
  ...
  for (const key of ['pageFileMap', 'pageDepMap', 'templateDepMap', 'pagesFileDepMap', 'layoutPageMap', 'layoutDepMap', 'layoutFileMap']) {
