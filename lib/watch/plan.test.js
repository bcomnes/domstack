/**
 * @import { WatchSnapshot, WatchPlan } from './plan.js'
...
test('settings and untracked changes produce inspectable full, page, and skip plans', () => {
  const { state } = fixture()
  for (const name of ['global.vars.js', 'esbuild.settings.js', 'markdown-it.settings.js']) {
...
test('failed page builds retry fully before incomplete maps or browser entries can skip the work', () => {
  ...
  for (const name of ['global.vars.js', 'markdown-it.settings.js']) {
    assert.equal(planWatchEvent(failed, classifyWatchEvent('change', `/site/${name}`)).kind, 'full')
  }
...
test('snapshots without optional dependency analysis fields remain compatible', () => {
  ...
  const { settingsDepPaths, dependencyAnalysisFailed, esbuildDepPaths, ...legacy } = state
...
test('settings roots and dependencies always rebuild fully and reset inputs regardless of shared roles', () => {
  const cases = /** @type {const} */ ([
    ['global.vars.js', false, 'global-vars-changed'],
    ['markdown-it.settings.js', false, 'markdown-settings-changed'],
    ['esbuild.settings.js', false, 'global-config-changed'],
    ['vars-helper.js', true, 'global-config-changed'],
    ['markdown-helper.js', true, 'global-config-changed'],
    ['client.jsx', true, 'global-config-changed'],
    ['page.md', true, 'global-config-changed'],
    ['domstack-manifest.settings.js', true, 'global-config-changed'],
  ])
  for (const type of /** @type {const} */ (['change', 'added', 'removed'])) {
    for (const [name, dependency, reason] of cases) {
      const { state, home, other, template, owner } = fixture()
      const filepath = `/site/${name}`
      if (dependency) state.settingsDepPaths.add(filepath)
      const event = classifyWatchEvent(type, filepath)
      assert.equal(planWatchEvent(state, event).kind, 'full', `${type} ${name}`)
      assert.equal(planWatchBatch(state, [event]).inputChanges.resetReason, reason, `${type} ${name}`)

      state.settingsDepPaths.add(filepath)
      ...
      const before = structuredClone(state)
      assert.equal(planWatchEvent(state, event).kind, 'full', `shared ${type} ${name}`)
      const batch = planWatchBatch(state, [event])
      assert.equal(batch.plan.kind, 'full', `shared ${type} ${name}`)
      assert.deepEqual(batch.inputChanges, {
        upsertedPaths: [home.pageFile.filepath, other.pageFile.filepath],
        resetReason: reason,
        events: [event],
      }, `shared ${type} ${name}`)
      assert.deepEqual(state, before)
      ...
      state.globalDataDepPaths.add(filepath)
      assert.equal(planWatchBatch(state, [event]).inputChanges.resetReason, 'global-data-changed', `global data ${type} ${name}`)
    }
  }
})
...
test('large batches accumulate selections once while retaining order and duplicate events', () => {
  const { state } = fixture()
  const pages = Array.from({ length: 1000 }, (_, i) => page(`post-${i}`))
  state.siteData.pages = pages
  state.pageFileMap = new Map(pages.map(page => [page.pageFile.filepath, page]))
  const events = pages.map(page => classifyWatchEvent('change', page.pageFile.filepath))
  events.push(...events.slice().reverse())
  const before = structuredClone({ state, events })
  const batch = planWatchBatch(state, events)
  assert.deepEqual(scope(batch.plan), [pages.map(page => page.pageFile.filepath), [], []])
  if (batch.plan.kind !== 'pages') throw new Error('Expected a page plan')
  assert.deepEqual(batch.plan.pages, pages)
  assert.deepEqual(batch.inputChanges.upsertedPaths, pages.map(page => page.pageFile.filepath))
  assert.deepEqual(batch.inputChanges.events, events)
  assert.notEqual(batch.inputChanges.events, events)
  assert.deepEqual({ state, events }, before)
})

test('full-plan messages preserve dominance and unreliable events replace earlier reasons', () => {
  const { state } = fixture()
  const pageEvent = classifyWatchEvent('change', '/site/page.md')
  const settings = classifyWatchEvent('change', '/site/global.vars.js')
  const restart = classifyWatchEvent('added', '/site/client.jsx')
  const unknown = classifyWatchEvent('change', '/site/unknown.js')
  assert.deepEqual(planWatchBatch(state, [pageEvent, settings, restart]).plan, planWatchEvent(state, settings))
  assert.deepEqual(planWatchBatch(state, [pageEvent, restart, settings]).plan, {
    kind: 'full', message: 'Bundle membership changed, triggering full rebuild...',
  })
  assert.deepEqual(planWatchBatch(state, [settings, unknown, restart, pageEvent]).plan, {
    kind: 'full', message: '"unknown.js" cannot be routed reliably, triggering full rebuild...',
  })
})
...
test('single page plans preserve exact logging metadata and messages even when surrounded by skips', () => {
  ...
  for (const name of ['page.md', 'layout-helper.js', 'feed.template.js', 'global.data.js']) {
...
test('batches without source input selections do not enumerate source pages', () => {
  const { state } = fixture()
  Object.defineProperty(state.siteData.pages, Symbol.iterator, {
    value: () => assert.fail('source pages must not be iterated'),
  })
  const events = ['client.jsx', 'feed.template.js', 'archive.pages.js', 'archive-helper.js', 'global.data.js']
    .map(name => classifyWatchEvent('change', `/site/${name}`))
  for (const event of events) {
    assert.deepEqual(planWatchBatch(state, [event]).inputChanges.upsertedPaths, [])
  }
  assert.deepEqual(planWatchBatch(state, events).inputChanges.upsertedPaths, [])
})

test('source membership is enumerated once per batch and excludes generated pages', t => {
  const { state, home, other, owner } = fixture()
  state.siteData.pages.push({ ...page('generated'), generated: { pagesFile: owner } })
  const iterate = state.siteData.pages[Symbol.iterator].bind(state.siteData.pages)
  const scan = t.mock.fn(iterate)
  Object.defineProperty(state.siteData.pages, Symbol.iterator, { value: scan })
  const events = ['page-helper.js', 'global.vars.js', 'page.md']
    .map(name => classifyWatchEvent('change', `/site/${name}`))
  const batch = planWatchBatch(state, events)
  assert.equal(scan.mock.callCount(), 1)
  assert.deepEqual(batch.inputChanges.upsertedPaths, [other.pageFile.filepath, home.pageFile.filepath])
...
test('null filters dominate filtered scopes and failure resets override event reasons', () => {
  for (const flag of /** @type {const} */ (['pageBuildFailed', 'dependencyAnalysisFailed'])) {
    ...
    const events = ['page.md', 'feed.template.js', 'global.data.js', 'client.jsx'].map(name => classifyWatchEvent('change', `/site/${name}`))
    const batch = planWatchBatch(state, events)
...
test('global data roots need no map entry and shared imports reset on changes, additions, and removals', () => {
  for (const type of /** @type {const} */ (['change', 'added', 'removed'])) {
    const { state, other } = fixture()
    state.globalDataDepPaths.clear()
    const root = classifyWatchEvent(type, '/site/global.data.js')
    assert.equal(planWatchBatch(state, [root]).inputChanges.resetReason, 'global-data-changed')
    if (type === 'change') assert.deepEqual(scope(planWatchEvent(state, root)), [[], [], []])
    for (const name of ['helper.js', 'client.jsx', 'page.md']) {
      const filepath = `/site/${name}`
      state.globalDataDepPaths.add(filepath)
      state.esbuildDepPaths.add(filepath)
      state.pageDepMap.set(filepath, new Set([other]))
      const batch = planWatchBatch(state, [classifyWatchEvent(type, filepath)])
      assert.equal(batch.inputChanges.resetReason, 'global-data-changed', `${type} ${name}`)
    }
  }
})
...
test('reset reason uses the first applicable event after snapshot failure precedence', () => {
  ...
  assert.equal(planWatchBatch(state, [data, markdown]).plan.kind, 'full', 'settings force full output selection even after a global-data reset')
})
