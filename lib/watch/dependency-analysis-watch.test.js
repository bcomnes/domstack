 ...
 * @import { WatchDependencyIndex as DependencyIndex } from './dependency-index.js'
 */
...
describe('watch coordinator dependency analysis', { concurrency: false, timeout: 60_000 }, () => {
  ...
  test('every raw event dirties analysis before filtering or browser-only skips without a capturing page build', async t => {
    const { WatchDependencyIndex } = await import('./dependency-index.js')
    const trace = /** @type {string[]} */ ([])
    const record = WatchDependencyIndex.prototype.recordEvent
    const filter = WatchDependencyIndex.prototype.filterEvents
    /**
     * @this {DependencyIndex}
     * @param {Parameters<DependencyIndex['recordEvent']>[0]} event
     */
    function recordEvent (event) {
      trace.push('record:' + event.type)
      return record.call(this, event)
    }
    /**
     * @this {DependencyIndex}
     * @param {Parameters<DependencyIndex['filterEvents']>} args
     */
    function filterEvents (...args) {
      trace.push('filter')
      return filter.apply(this, args)
    }
    t.mock.method(WatchDependencyIndex.prototype, 'recordEvent', recordEvent)
    t.mock.method(WatchDependencyIndex.prototype, 'filterEvents', filterEvents)
    const site = await fixture(t, {
    ...
    for (const type of /** @type {const} */ (['change', 'add', 'unlink'])) {
      ...
      trace.length = 0
      site.resetReads()
      site.emit('untracked.txt', type)
      site.emit('untracked.txt', type)
      const recordedType = type === 'add' ? 'added' : type === 'unlink' ? 'removed' : 'change'
      assert.deepEqual(trace, ['record:' + recordedType, 'record:' + recordedType], 'duplicates are recorded synchronously, before filtering')
      await nextTurn()
      await site.dom.settled()
      assert.deepEqual(trace, ['record:' + recordedType, 'record:' + recordedType, 'filter'])
  ...
  test('failed page work forces fresh analysis on recovery before warm reuse resumes', async t => {
    ...
    assert.deepEqual(site.reads(), [], 'failed rendering cannot publish a new analysis baseline')
  ...
  test('watcher error during in-flight observation prevents publication and stays untrusted across later builds', async t => {
    ...
    for (const body of ['Recovery pass', 'Successful but untrusted', 'Still not retained']) {
      site.resetReads()
      await site.write('a/page.md', markdown(body))
      await site.deliver('a/page.md')
