...
import { esbuildSettingsNames, markdownItSettingsNames } from './file-conventions.js'
...
test('only esbuild settings edits warn that a process restart is needed', () => {
  const { state } = fixture()
  for (const name of esbuildSettingsNames) {
    for (const pageBuildFailed of [false, true]) {
      const plan = planWatchEvent({ ...state, pageBuildFailed }, classifyWatchEvent('change', `/site/${name}`))
      assert.equal(plan.kind, 'full')
      if (plan.kind !== 'full') throw new Error('Expected a full rebuild')
      assert.ok(plan.warning?.includes(`"${name}"`))
      assert.match(plan.warning ?? '', /Stop and restart DOMStack/)
      assert.match(plan.warning ?? '', /restarting esbuild alone does not reload/)
    }
  }
  for (const name of [...markdownItSettingsNames, 'global.vars.js', 'domstack-manifest.settings.js']) {
    const plan = planWatchEvent(state, classifyWatchEvent('change', `/site/${name}`))
    assert.ok(!('warning' in plan))
  }
  for (const type of /** @type {const} */ (['added', 'removed'])) {
    const plan = planWatchEvent(state, classifyWatchEvent(type, '/site/esbuild.settings.js'))
    assert.equal(plan.kind, 'full')
    assert.ok(!('warning' in plan), 'structural rediscovery remains unchanged')
  }
})
