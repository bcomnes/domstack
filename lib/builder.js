 ...
 * @property {boolean | { write?: boolean, filename?: string, exclude?: string[] } | undefined} [domstackManifest=true] - Configure writing the domstack manifest file. Programmatic builds always return it.
...
export async function builder (src, dest, opts) {
  ...
  const domstackManifestOptions = await resolveDomstackManifestOptions({
    domstackManifestSettingsPath: siteData?.domstackManifestSettings?.filepath,
    opts,
  })
  const esbuildOpts = withResolvedDomstackManifestFilename(opts, domstackManifestOptions.filename)
  ...
  const [
    ...
    buildEsbuild(src, dest, siteData, esbuildOpts),
    opts.static
  ...
  const domstackManifest = await buildDomstackManifest({
    dest,
    records: baseOutputRecords,
    options: domstackManifestOptions,
  })

  results.domstackManifest = domstackManifest

  if (shouldWriteDomstackManifest(opts)) {
    await writeDomstackManifest(dest, domstackManifest, domstackManifestOptions.filename)
  }
...
/**
 * Let esbuild's DOMSTACK_MANIFEST_URL define observe a filename supplied by
 * domstack-manifest.settings.* without changing the public build options shape.
 *
 * @param {DomStackOpts} opts
 * @param {string | undefined} filename
 * @returns {DomStackOpts}
 */
function withResolvedDomstackManifestFilename (opts, filename) {
  if (opts.domstackManifest === false || !filename) return opts
