 ...
 * @import { BuildOutputManifest } from './build-output-manifest/index.js'
...
import {
  buildOutputManifest,
  resolveOutputManifestOptions,
  shouldWriteOutputManifest,
  writeOutputManifest,
} from './build-output-manifest/index.js'
 ...
 * @property {boolean | { write?: boolean, filename?: string, exclude?: string[] } | undefined} [outputManifest=true] - Configure writing the domstack output manifest file. Programmatic builds always return it.
 ...
 * @property {BuildOutputManifest} [outputManifest]
...
export async function builder (src, dest, opts) {
  ...
  const outputManifestOptions = await resolveOutputManifestOptions({
    globalVarsPath: siteData?.globalVars?.filepath,
    opts,
  })
  ...
  results.outputManifest = outputManifest

  if (shouldWriteOutputManifest(opts)) {
    await writeOutputManifest(dest, outputManifest, outputManifestOptions.filename)
  }
...
/**
 * @param  {...{ report?: { outputs?: BuildOutputRecord[] } } | null | undefined} results
