 ...
 * @param {{ watch?: boolean, caseInsensitive?: boolean, promoteOutputs?: (claims: OutputClaim[], write: () => Promise<void>) => Promise<void> }} [internal]
...
export async function builder (src, dest, opts, internal = {}) {
  ...
  try {
    ...
    const write = async () => {
      await mkdir(dest, { recursive: true })
      await cp(stageDest, await realpath(dest), { recursive: true, force: true })
    }
    if (internal.promoteOutputs) await internal.promoteOutputs(results.outputClaims ?? [], write)
    else await write()
    delete results.outputClaims
    return results
  } catch (error) {
