 ...
 * @property {string[]} upsertedPaths
 * @property {WatchEvent[]} events
 */
 ...
 * @property {'delta'} kind
 * @property {PageData<T, U, any, any>[]} upserted - Current initialized pages, identified by their normalized absolute pageInfo.pageFile.filepath.
 * @property {string[]} removed - Normalized absolute source-page paths no longer eligible.
 * @property {WatchEvent[]} events
 */
...
export function createGlobalDataState ({ pages, previousGlobalDataBaseline, globalDataInputChanges }) {
  const sourcePaths = [...new Set(pages.map(page => resolve(page.pageInfo.pageFile.filepath)))]
  ...
  const previousPaths = new Set(previousGlobalDataBaseline?.sourcePaths.map(path => resolve(path)))
    ...
    changes: reset
      ...
      : {
          ...
          upserted: pages.filter(page => {
            ...
            return upsertedPaths.has(path) || !previousPaths.has(path)
          }),
          removed: [...previousPaths].filter(path => !sourcePathSet.has(path)),
          events,
        },
    setState (next) {
