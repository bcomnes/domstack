...
test('shared mutable storage is rejected throughout nested state without replacing a valid candidate', () => {
  ...
  const states = [
    ...
    new Map([['histogram', createHistogram()]]),
  ...
  for (const state of states) {
    assert.throws(() => transaction.context.setState(state), /shared.*state/)
    assert.equal(transaction.getBaseline().state, candidate)
    assert.throws(() => createGlobalDataState({
      pages: [], previousGlobalDataBaseline: { state, sourceIds: [] },
    }), /global\.data previous state.*shared.*state/)
  }
})
...
test('worker validation errors remain actionable and never emit a candidate', async t => {
  for (const [expression, expected] of /** @type {const} */ ([
    ...
    ['new WebAssembly.Memory({ initial: 1, maximum: 1, shared: true })', /shared.*state/],
    ['(await import("node:perf_hooks")).createHistogram()', /shared.*state/],
    ['(await import("node:perf_hooks")).monitorEventLoopDelay()', /shared.*state/],
    ['new (await import("node:net")).BlockList()', /shared.*state/],
  ])) {
    const { src, dest } = await fixture(t, `export default async ({ setState }) => { setState(${expression}); return { summary: 'stable' } }`)
...
test('shared native state cannot cross into a worker callback and mutate the retained baseline', async t => {
  const { src, dest } = await fixture(t, `export default ({ previousState }) => {
    new Uint8Array(previousState.memory.buffer)[0] = 42
    return { summary: 'unchanged' }
  }`)
  const memory = new WebAssembly.Memory({ initial: 1, maximum: 1, shared: true })
  const result = await buildPages(src, dest, await identifyPages(src), {
    trackWatchDependencies: true,
    previousGlobalDataBaseline: { state: { memory }, sourceIds: ['page.js'] },
  })
  assert.equal(new Uint8Array(memory.buffer)[0], 0)
  assert.equal(result.errors.length, 1)
  const error = result.errors[0]
  assert.ok(error instanceof Error)
  assert.match(error.message, /global\.data previous state.*shared.*state/)
  assert.equal(Object.hasOwn(result.report, 'globalDataBaseline'), false)
})

test('watch builds without global data do not emit an index baseline', async t => {
  const { src, dest } = await fixture(t, 'export default {}')
  await rm(join(src, 'global.data.js'))
  await writeFile(join(src, 'page.js'), 'export default () => "Home"')
  const result = await buildPages(src, dest, await identifyPages(src), { trackWatchDependencies: true })
  assert.deepEqual(result.errors, [])
  assert.equal(Object.hasOwn(result.report, 'globalDataBaseline'), false)
})
