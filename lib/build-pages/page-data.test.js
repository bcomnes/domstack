...
test.describe('PageData.vars', () => {
  test('returns a cached, frozen vars object and invalidates when sources change', async () => {
    ...
    try {
      ...
      pd.layoutVars = { ...pd.layoutVars, fromLayout: 'updated layout' }
...
`)
  ...
  test('unions page and layout data dependencies outside resolved vars', async () => {
    ...
    try {
      await writeFile(mdFile, '---\ndataDependencies: [feedItems]\n---\n# Test\n')
      await writeFile(pageVarsFile, 'export default { dataDependencies: [\'recentPosts\'] }\n')

      const layout = {
        ...fakeLayout,
        vars: { dataDependencies: ['navigation'] },
      }
      /** @type {PageData<TestVars, string, string>} */
      const pd = new PageData({
        pageInfo: {
      ...
      assert.deepEqual(pd.dataDependencies, ['feedItems', 'navigation', 'recentPosts'])
      assert.deepEqual(Object.keys(pd.data), ['feedItems', 'navigation', 'recentPosts'])
      assert.equal('dataDependencies' in pd.vars, false)
    } finally {
      await rm(dir, { recursive: true, force: true })
    }
  })
