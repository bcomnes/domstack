...
for (const type of /** @type {const} */ (['md', 'html'])) {
  ...
  test(`${type} loads no Handlebars modules until enabled in a fresh process`, async t => {
    const { pageInfo } = await fixture(t, type, '{{vars.title}}')
    const result = spawnSync(process.execPath, ['--input-type=module', '--eval', `
      ...
      assert.equal(require.extensions['.hbs'], undefined)
      ...
      const built = await builder({ pageInfo })
      assert.deepEqual(loaded(), [], 'preparing a page must not load Handlebars')
      const vars = { handlebars: false, title: 'Enabled' }
      const context = { vars, data: {}, page: pageInfo }
      assert.match(await built.pageLayout(context), /{{vars.title}}/)
      assert.deepEqual(loaded(), [], 'disabled rendering must not load Handlebars')
      assert.equal(require.extensions['.hbs'], undefined)
      vars.handlebars = true
      assert.match(await built.pageLayout(context), /Enabled/)
      assert.ok(loaded().includes(require.resolve('handlebars')))
      assert.equal(typeof require.extensions['.hbs'], 'function')
...
test('Markdown and HTML use registered helpers on the shared default Handlebars singleton', async t => {
  const { default: Handlebars } = await import('handlebars')
  const helper = 'domstackLazyHandlebarsTest'
  t.after(() => Handlebars.unregisterHelper(helper))
  Handlebars.registerHelper(helper, value => `First ${value}`)
  const source = `{{${helper} vars.title}}`
  const md = await fixture(t, 'md', source)
  const html = await fixture(t, 'html', source)
  const vars = { handlebars: true, title: 'Example' }
  assert.deepEqual(await Promise.all([md.render(vars), html.render(vars)]), [
    '<p>First Example</p>\n',
    'First Example',
  ])
  Handlebars.registerHelper(helper, value => `Updated ${value}`)
  vars.title = 'Live'
  assert.deepEqual(await Promise.all([html.render(vars), md.render(vars)]), [
    'Updated Live',
    '<p>Updated Live</p>\n',
  ])
})
