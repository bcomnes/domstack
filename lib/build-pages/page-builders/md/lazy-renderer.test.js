...
for (const mode of ['omitted options', 'standalone', 'build resolver']) {
  test(`${mode}: preparation exposes metadata without initializing a renderer and captures source`, async t => {
    ...
    const options = mode === 'build resolver' ? { getMarkdownRenderer: createMdResolver() } : {}
    const prepared = await mdBuilder(mode === 'omitted options' ? { pageInfo } : { pageInfo, options })
    assert.equal(prepared.vars['title'], 'Original **title**')
    assert.equal(prepared.vars['category'], 'notes')
    ...
    assert.ok(use.mock.callCount() > 0, 'first render initializes the actual default renderer')
    const pluginCalls = use.mock.callCount()
    await rm(pageInfo.pageFile.filepath)
    assert.equal(await prepared.pageLayout(context), html, 'later renders need no source read')
    assert.equal(use.mock.callCount(), pluginCalls, 'later renders reuse the renderer')
  })
}
...
test('concurrent first renders share one default renderer per build, not across builds', async t => {
  ...
  assert.ok(use.mock.callCount() > 0)
  assert.equal(new Set(use.mock.calls.map(call => call.this)).size, 1)
  ...
  const pluginCalls = use.mock.callCount()
  await Promise.all(pages.map(page => page.pageLayout(context)))
  assert.equal(use.mock.callCount(), pluginCalls)

  const next = await mdBuilder({ pageInfo, options: { getMarkdownRenderer: createMdResolver() } })
  assert.equal(use.mock.callCount(), pluginCalls, 'a fresh build is also lazy')
  await next.pageLayout(context)
  assert.equal(new Set(use.mock.calls.map(call => call.this)).size, 2)
  assert.equal(new Set(render.mock.calls.map(call => call.this)).size, 2)
})
...
test('concurrent lazy initialization failures are shared and evicted so prepared pages can retry', async t => {
  ...
  assert.match(html, /Original <strong>title<\/strong>/)
  assert.match(html, /Original body\./)
  assert.ok(use.mock.callCount() > 0, 'retry constructs a new renderer')
  assert.equal(new Set(use.mock.calls.map(call => call.this)).size, 1)
  const pluginCalls = use.mock.callCount()
  ...
  assert.equal(use.mock.callCount(), pluginCalls, 'siblings reuse the successful retry')
  const shared = await options.getMarkdownRenderer(null)
  assert.ok(render.mock.calls.every(call => call.this === shared))
  assert.equal(render.mock.callCount(), pages.length)
...
test('standalone pages initialize independent renderers even when first rendered concurrently', async t => {
  ...
  assert.equal(use.mock.callCount(), 0)
  await Promise.all([first.pageLayout(context), second.pageLayout(context)])
  assert.equal(new Set(use.mock.calls.map(call => call.this)).size, 2)
  const [firstRender, secondRender] = render.mock.calls
  assert.ok(firstRender && secondRender)
  assert.notEqual(firstRender.this, secondRender.this)
...
test('an initialized default page parses synchronously on subsequent overlapping renders', async t => {
  const { pageInfo, context } = await fixture(t)
  ...
  const parse = t.mock.method(renderer, 'parse')

  const first = page.pageLayout(context)
  assert.equal(parse.mock.callCount(), 1, 'parsing happens before pageLayout returns its promise')
  state = 'second'
