 ...
 * @param  {{ vars?: Record<string, unknown> }} vars to expose to handlebars
...
export async function renderMd (mdUnparsed, vars, md, settingsPath) {
  ...
  const template = compileHandlebars(mdUnparsed, vars)
  return rewriteLinks(md.render(template ? template(vars) : mdUnparsed))
}
