...
import { createOutputRecord } from '../../build-output-manifest/index.js'
 ...
 * @property {TemplateOutputReport[]} outputFiles - Array of files the template output to
 * @property {BuildOutputRecord[]} outputRecords - Build output records emitted by the template.
 ...
 * @param {TemplateReport} params.templateReport
 */
async function writeTemplateOutput ({
  ...
  templateReport.outputs.push(outputName)
  const outputRelname = toPosix(relative(dest, filepath))
  templateReport.outputFiles.push({
    outputName,
    outputRelname,
    filepath,
  })
  templateReport.outputRecords.push(createOutputRecord({
    dest,
    filepath,
    outputRelname,
    kind: 'template',
    sourceRelname: templateReport.templateInfo.templateFile.relname,
    templatePath: templateReport.templateInfo.path,
  }))
}
