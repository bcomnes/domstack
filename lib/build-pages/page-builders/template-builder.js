 ...
 * @returns {string | TemplateOutputOverride | TemplateOutputOverride[] | Promise<string | TemplateOutputOverride | TemplateOutputOverride[]>} The results of a template build.
 */

/**
 * Promise-returning specialization of TemplateFunction.
 * @template {Record<string, any>} T - The type of variables for the template.
 * @template {object} [D=Record<string, unknown>] - Declared global data.
 * @callback AsyncTemplateFunction
 * @param {TemplateFunctionParams<T, D>} params
 ...
 * @param  {OutputRegistry} [params.outputRegistry] - Shared output ownership for this page phase.
...
async function writeTemplateOutput ({
  ...
  const filepath = resolve(fileDir, outputName.replaceAll('\\', '/'))
  ...
  outputRegistry.claim(outputRelname, {
    id: `template:${template.templateFile.filepath}`,
    type: 'template',
    path: template.templateFile.relname,
  })
