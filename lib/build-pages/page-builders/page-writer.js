 ...
 * @property {AdditionalOutputsFunction<T, D> | undefined} [additionalOutputs] - Optional build-only additional-output hook.
 */
 ...
 * @param {((output: DomstackManifestRecord) => void) | undefined} [params.onAdditionalOutput] - Record emitted sidecars even if a later yield fails.
...
export async function pageWriter ({
  ...
  const additionalOutputs = await writeAdditionalOutputs({
    ...
    additionalOutputs: page.collectAdditionalOutputs(),
    onOutput: onAdditionalOutput,
  })
