 ...
 * @import { AdditionalOutputsFunction, AdditionalOutputProvenance } from '../additional-outputs.js'
 */

import { join } from 'path'
import { writeFile, mkdir } from 'fs/promises'
import { createDomstackManifestRecord } from '../../domstack-manifest/index.js'
import { writeAdditionalOutputs } from './additional-output-writer.js'
 ...
 * @template [U=any] U - The return type of the page function (defaults to any)
 * @template {object} [D=Record<string, unknown>] - Declared global data.
 * @typedef {PageFunction<T, U, D>} InternalPageFunction
 */
 ...
 * @property {InternalPageFunction<T, U, D>} pageLayout - The function that returns the rendered page
 * @property {AdditionalOutputsFunction<T, D>} [additionalOutputs] - Optional build-only additional-output hook.
 */
 ...
 * @param {(outputRelname: string, provenance?: AdditionalOutputProvenance) => void} [params.claimAdditionalOutput]
 * @returns {Promise<{ pageFilePath: string, outputs: DomstackManifestRecord[] }>}
 */
export async function pageWriter ({
  dest,
  page,
  claimAdditionalOutput,
}) {
  ...
  outputs.push(...await writeAdditionalOutputs({
    dest,
    pageFilePath,
    pageInfo: page.pageInfo,
    additionalOutputs: await page.collectAdditionalOutputs(),
    claimOutput: claimAdditionalOutput,
  }))
