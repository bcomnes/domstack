 ...
 * @param {Array<{outputName: string, content: string}>} params.additionalOutputs
 * @returns {Promise<DomstackManifestRecord[]>}
 */
export async function writeAdditionalOutputs ({ dest, pageFilePath, pageInfo, additionalOutputs }) {
  const planned = additionalOutputs.map(output => {
    ...
    return { ...resolveAdditionalOutputPath(dest, pageFilePath, output.outputName), content: output.content }
  })
  // Check every path before writing this page's batch.
