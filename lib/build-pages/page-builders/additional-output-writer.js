 ...
 * @param {Iterable<{outputName: string, content: string}> | AsyncIterable<{outputName: string, content: string}>} params.additionalOutputs
 * @param {((output: DomstackManifestRecord) => void) | undefined} [params.onOutput]
 * @returns {Promise<DomstackManifestRecord[]>}
 */
export async function writeAdditionalOutputs ({ dest, pageFilePath, pageInfo, additionalOutputs, onOutput }) {
  const records = []
  for await (const output of additionalOutputs) {
    if (typeof output.content !== 'string') throw new TypeError('Additional output content must be a string')
    const { filepath, outputRelname } = resolveAdditionalOutputPath(dest, pageFilePath, output.outputName)
    await assertWritablePath(dest, filepath)
