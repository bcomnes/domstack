...
import {
  ...
  WatchDependencyTracker as WatchDependencyTrackerClass,
 ...
 * @property {PageData<any, any, any, any>[]} pages - Fully initialized source-backed pages, before generated pages are created.
 */
 ...
 * @returns {GeneratedPageDefinition<T, U, D> | GeneratedPageDefinition<T, U, D>[] | AsyncIterable<GeneratedPageDefinition<T, U, D>> | Promise<GeneratedPageDefinition<T, U, D> | GeneratedPageDefinition<T, U, D>[] | AsyncIterable<GeneratedPageDefinition<T, U, D>>>}
 */
...
async function resolveGeneratedPageInfos ({ siteData, factoryVars, globalData, pagesFileFilterSet, buildDrafts, watchDependencyTracker }) {
  ...
  for (const pagesFile of siteData.pagesFiles ?? []) {
    ...
    try {
      ...
      const dataDependencies = resolveDataDependencies(
        importResults.dataDependencies,
...
export async function buildPagesDirect (_src, dest, siteData, opts) {
  ...
  if (Object.hasOwn(globalVars, 'dataDependencies')) {
    throw new Error('dataDependencies is page and layout metadata and cannot be declared in global vars')
  }
