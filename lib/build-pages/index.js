 ...
 * @property {(report: PageBuildStepResult, write: () => Promise<void>) => Promise<void>} [promoteOutputs]
 */
  ...
  try {
    ...
    try {
      if (buildReport.errors.length === 0) {
        const write = () => promotePageOutputs(dest, buildReport)
        if (opts?.promoteOutputs) await opts.promoteOutputs(buildReport, write)
        else await write()
      }
      return buildReport
    } finally {
 ...
 */
async function promotePageOutputs (dest, buildReport) {
