 ...
 * @returns {Promise<PageBuildStepResult & { postVarsPagePaths: string[] }>}
 */
export function buildPages (src, dest, siteData, _opts, buildPagesOpts = {}) {
  return new Promise((resolve, reject) => {
    ...
    worker.once('message', message => {
      ...
      /** @type {PageBuildStepResult & { postVarsPagePaths: string[] }} */
      const buildReport = {
        type: workerReport.type,
        report: workerReport.report,
        errors: [],
        warnings: workerReport.warnings ?? [],
        postVarsPagePaths: workerReport.postVarsPagePaths ?? [],
      }
...
export async function buildPagesDirect (src, dest, siteData, _opts, buildPagesOpts = {}) {
  ...
  const pages = await pMap(siteData.pages, async (pageInfo) => {
    ...
    // Track pages with postVars for incremental rebuild decisions
    if (pageData.postVars !== null) {
      result.postVarsPagePaths.push(pageInfo.pageFile.filepath)
      result.report.postVarsPagePaths.push(pageInfo.pageFile.filepath)
    }
  ...
  // Apply pageFilter if provided — only render pages whose filepath is in the filter set
  const pagesToBuild = pageFilterPaths
    ? pages.filter(page => pageFilterPaths.has(page.pageInfo.pageFile.filepath))
    : pages

  // Apply templateFilter if provided — only render templates whose filepath is in the filter set
  const templatesToBuild = templateFilterPaths
    ? siteData.templates.filter(template => templateFilterPaths.has(template.templateFile.filepath))
    : siteData.templates

  await Promise.all([
    pMap(pagesToBuild, async (page) => {
      try {
    ...
    pMap(templatesToBuild, async (template) => {
      try {
