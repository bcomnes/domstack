...
const CONCRETE_PAGES_COLLECTION = 'concrete-pages'
const ALL_PAGES_COLLECTION = 'all-pages'
...
export async function buildPagesDirect (_src, dest, siteData, opts) {
  ...
  const trackedConcretePages = watchDependencyTracker.trackPageCollection(
    concretePages.map(page => watchDependencyTracker.trackPageData(page)),
    CONCRETE_PAGES_COLLECTION
  )
  const changedPageCollections = new Set()
  if (watchDependencyTracker.updatePageCollectionFingerprint(
    CONCRETE_PAGES_COLLECTION,
    concretePages,
    opts?.previousWatchDependencies?.pageCollectionFingerprints
  )) {
    changedPageCollections.add(CONCRETE_PAGES_COLLECTION)
  }
  ...
  const pagesFactoryVars = watchDependencyTracker.trackGlobalDataVars({ ...globalVars, ...globalDataVars })
  ...
  const trackedAllPages = watchDependencyTracker.trackPageCollection(allPages, ALL_PAGES_COLLECTION)
  if (watchDependencyTracker.updatePageCollectionFingerprint(
    ALL_PAGES_COLLECTION,
    [...concretePages, ...generatedPages],
    opts?.previousWatchDependencies?.pageCollectionFingerprints
  )) {
    changedPageCollections.add(ALL_PAGES_COLLECTION)
  }
  ...
  if (!fullBuild) {
    /** @type {Map<string, PageData<any, any, any>>} */
    const changedPages = new Map()
    for (const page of concretePages) {
      if (pageFilterSet?.has(page.pageInfo.pageFile.filepath)) {
        changedPages.set(page.pageInfo.pageFile.filepath, page)
      }
    }
    for (const page of generatedPages) {
      changedPages.set(page.pageInfo.outputRelname, page)
    }
  ...
  const templateGlobalVars = watchDependencyTracker.trackGlobalDataVars({ ...globalVars, ...globalDataVars })
    ...
    pMap(pagesToWrite, async (page) => {
      try {
        ...
        const buildResult = await pageWriter({
          dest,
          page,
          pages: trackedAllPages,
          renderFullPage: () => watchDependencyTracker.runWithConsumer(
            'page',
            pageConsumerKey,
            () => page.renderFullPage({ pages: trackedAllPages }),
