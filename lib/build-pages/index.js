 ...
 * @import { GlobalDataBaseline, GlobalDataInputChanges, GlobalDataChanges } from './global-data-state.js'
 */
 ...
 * @property {PageData<T, U, any, any>[]} pages - Fully initialized source-backed pages, before generated pages are created. Identities are normalized absolute pageInfo.pageFile.filepath values.
...
export async function buildPagesDirect (_src, dest, siteData, opts) {
  ...
  const concretePages = await pMap(siteData.pages, pageInfo => initPageData({
    ...pageInfo,
    pageFile: { ...pageInfo.pageFile, filepath: resolve(pageInfo.pageFile.filepath) },
  }), { concurrency: MAX_CONCURRENCY })
  ...
  const globalDataState = createGlobalDataState({
    pages: concretePages,
    previousGlobalDataBaseline: opts?.previousGlobalDataBaseline,
    globalDataInputChanges: opts?.globalDataInputChanges,
  })
  ...
  if (opts?.trackWatchDependencies && result.errors.length === 0) result.report.globalDataBaseline = globalDataState.getBaseline()
  return result
}
