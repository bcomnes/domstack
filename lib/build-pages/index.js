...
export async function buildPagesDirect (_src, dest, siteData, opts) {
  ...
  if (watchDependencyTracker.updatePageCollectionFingerprint(
    ...
    changedPageCollections.add('concrete-pages')
  }
  ...
  const trackedAllPages = watchDependencyTracker.trackPageCollection(allPages, 'all-pages')
  if (watchDependencyTracker.updatePageCollectionFingerprint(
    'all-pages',
    [...concretePages, ...generatedPages],
    opts?.previousWatchDependencies?.pageCollectionFingerprints
  )) {
    changedPageCollections.add('all-pages')
  }
  ...
  if (!fullBuild) {
    const changedPages = new Map([
      ...concretePages
        ...
        .map(page => /** @type {[string, PageData<any, any, any>]} */ ([page.pageInfo.pageFile.filepath, page])),
      ...generatedPages.map(page => /** @type {[string, PageData<any, any, any>]} */ ([page.pageInfo.outputRelname, page])),
    ])
