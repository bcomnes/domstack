 ...
 * @import { OutputClaim, OutputOwner } from '../output-registry.js'
 */
 ...
 * @property {OutputClaim[] | undefined} [newClaims] - Claims produced by the selected owners.
 * @property {string[]} [replacedOwnerIds]
 */
 ...
 * @property {OutputClaim[] | null | undefined} [previousOutputClaims] - Output ownership from the latest successful build.

 * @property {boolean} [caseInsensitive]
 * @property {(report: PageBuildStepResult, write: () => Promise<void>, preflight: (removablePaths: string[]) => Promise<void>) => Promise<void>} [promoteOutputs]
 */
 ...
 * @param {OutputRegistry} params.outputRegistry
 * @returns {Promise<PageInfo[]>}
 */
async function resolveGeneratedPageInfos ({ siteData, factoryVars, globalData, pagesFileFilterSet, buildDrafts, watchDependencyTracker, outputRegistry }) {
  /** @type {PageInfo[]} */
  ...
  for (const pagesFile of siteData.pagesFiles ?? []) {
    ...
    try {
      ...
      for (const [index, definition] of definitions.entries()) {
        ...
        outputRegistry.claim(generatedPageInfo.outputRelname, pageOutputOwner(generatedPageInfo))
 ...
 * @type {PageBuildStep}
 */
export async function buildPages (src, dest, siteData, opts) {
  await mkdir(dest, { recursive: true })
  const stageDest = await mkdtemp(join(dest, '.domstack-pages-'))
  try {
    ...
    const workerOpts = {
      ...
      previousOutputClaims: opts?.previousOutputClaims,
      caseInsensitive: opts?.caseInsensitive ?? await isCaseInsensitiveDest(dest),
    }

    const buildReport = await new Promise((resolve, reject) => {
      const worker = new Worker(join(__dirname, 'worker.js'), {
        workerData: { src, dest: stageDest, siteData, opts: workerOpts },
      })
      ...
      worker.once('message', message => {
        ...
        if (workerReport.errors.length > 0) {
          report.errors = workerReport.errors.map(({ error, errorData = {} }) => {
    ...
    try {
      if (buildReport.errors.length === 0) {
        ...
        const preflight = async (/** @type {string[]} */ removablePaths) => {
          unchanged = await prepareAdditionalOutputPromotion(dest, buildReport.outputs, removablePaths)
        }
        const write = () => promotePageOutputs(dest, buildReport, unchanged)
        if (opts?.promoteOutputs) await opts.promoteOutputs(buildReport, write, preflight)
        else {
          await preflight([])
          await write()
        }
      ...
      for (const output of buildReport.outputs) output.filepath = resolve(dest, output.outputRelname)
      for (const page of buildReport.report.pages) page.pageFilePath = resolve(dest, relative(stageDest, page.pageFilePath))
    }
  } finally {
...
/**
 * Promote a successful page phase only after every output has rendered and all
 * claims have been validated. Render/conflict failures do not alter the real dest;
 * an I/O failure during promotion can leave partially updated files.
 *
 * @param {string} dest
 * @param {PageBuildStepResult} buildReport
 * @param {Set<string>} unchanged
 */
async function promotePageOutputs (dest, buildReport, unchanged) {
  const resolvedDest = resolve(dest)
  for (const output of buildReport.outputs) {
    const target = resolve(resolvedDest, output.outputRelname)
    assertInsideDest(resolvedDest, target)
    if (!unchanged.has(output.outputRelname)) {
      await mkdir(dirname(target), { recursive: true })
      await copyFile(output.filepath, target)
    }
...
export async function buildPagesDirect (_src, dest, siteData, opts) {
  ...
  const outputRegistry = new OutputRegistry(opts?.previousOutputClaims ?? [], {
  ...
  // Select once, after global-data invalidation has expanded the filters. Include
  // deleted producers and factories that now emit no files, not just render results.
  const replacedOwnerIds = new Set([
    ...(opts?.previousOutputClaims ?? []).filter(({ owner }) => {
      if (owner.id.startsWith('page:')) return !pageFilterSet || pageFilterSet.has(owner.id.slice(5))
      if (owner.id.startsWith('template:')) return !templateFilterSet || templateFilterSet.has(owner.id.slice(9))
      if (owner.id.startsWith('pages-file:')) return !pagesFileFilterSet || pagesFileFilterSet.has(owner.id.slice(11))
      return false
    }).map(({ owner }) => owner.id),
    ...siteData.pages.filter(page => !pageFilterSet || pageFilterSet.has(page.pageFile.filepath)).map(page => pageOutputOwner(page).id),
    ...siteData.templates.filter(template => !templateFilterSet || templateFilterSet.has(template.templateFile.filepath)).map(template => `template:${template.templateFile.filepath}`),
    ...(siteData.pagesFiles ?? []).filter(file => !pagesFileFilterSet || pagesFileFilterSet.has(file.pagesFile.filepath)).map(file => `pages-file:${file.pagesFile.filepath}`),
  ])
  outputRegistry.releaseOwnerIds(replacedOwnerIds)
  result.report.replacedOwnerIds = [...replacedOwnerIds]

  let generatedPageInfos = /** @type {PageInfo[]} */ ([])
  try {
    for (const page of siteData.pages) {
      if (replacedOwnerIds.has(pageOutputOwner(page).id) || !opts?.previousOutputClaims) {
        outputRegistry.claim(page.outputRelname, pageOutputOwner(page))
      }
    }
  ...
  try {
    for (const page of pagesToWrite) {
      ...
      if (page.pageInfo.workers && Object.values(page.pageInfo.workers).some(worker => worker.outputRelname)) {
        outputRegistry.claim(join(page.pageInfo.path, 'workers.json'), owner)
      }
    }
  } catch (err) {
    result.errors.push(serializeBuildError(err))
    return result
  }
    ...
    pMap(pagesToWrite, async (page) => {
      try {
        const buildResult = await pageWriter({
          dest,
          page,
          claimAdditionalOutput: (outputRelname, provenance) => {
            const owner = pageOutputOwner(page.pageInfo)
            outputRegistry.claim(outputRelname, {
              ...owner,
              path: provenance ? `${owner.path} (${provenance.kind} additionalOutputs: ${provenance.source})` : owner.path,
            })
          },
        })
  ...
  result.report.newClaims = outputRegistry.snapshot()
    .filter(claim => replacedOwnerIds.has(claim.owner.id))
    .map(claim => claim.owner.id.startsWith('pages-file:')
      // Persist the factory's source identity, not its transient definition index.
      ? { ...claim, owner: { ...claim.owner, path: claim.owner.path.replace(/#\d+$/, '') } }
      : claim)
  if (opts?.trackWatchDependencies) {
 ...
 * @returns {OutputOwner}
 */
function pageOutputOwner (pageInfo) {
  const pagesFile = pageInfo.generated?.pagesFile.pagesFile
  return pagesFile
    ? { id: `pages-file:${pagesFile.filepath}`, type: 'page', path: pageInfo.pageFile.relname }
    : { id: `page:${pageInfo.pageFile.filepath}`, type: 'page', path: pageInfo.pageFile.relname }
}
