 ...
 * @import { getMd } from '../page-builders/md/get-md.js'
 * @import { PreparedMarkdown } from '../source-preparation/markdown.js'
 */
 ...
 * @property {typeof getMd} [getMarkdownRenderer] - Internal build-scoped renderer resolver; never transferred to workers
 * @property {((pageInfo: PageInfo) => Promise<PreparedMarkdown>) | undefined} [prepareMarkdown] - Internal source preparation resolver; never transferred to workers
 */
 ...
 * @property {D} data - Global data declared by the page itself, independent of its layouts.
...
/**
 * Page function for rendering inner content, synchronously or asynchronously.
 *
 * @template {Record<string, any>} T - The type of variables passed to the page function
 * @template [U=any] U - The return type of the page function (defaults to any)
 * @template {object} [D=Record<string, unknown>] - Declared global data.
 ...
 * @template {object} [D=Record<string, unknown>] - The page's declared global data.
 * @typedef PageBuilderResult
 * @property {Partial<T>} vars - Any variables resolved by the builder
 * @property {PageFunction<T, U, D>} pageLayout - The function that returns the rendered page
 * @property {PageOutputsFunction<T, D> | undefined} [pageOutputs] - Optional build-only page-output hook.
 */
...
export async function pageWriter ({
  ...
  page.outputRecords.length = 0
  const formattedPageOutput = await page.renderFullPage()
  await writePageOutputs({
    dest,
    pageFilePath,
    page,
    pageOutputs: page.collectPageOutputs(),
    outputCache,
  })
  ...
  page.outputRecords.unshift(createDomstackManifestRecord({
    dest,
  ...
  if (page.pageInfo?.workers) {
    ...
    if (Object.keys(workerMappings).length > 0) {
      ...
      page.outputRecords.push(createDomstackManifestRecord({
        dest,
  ...
  return { pageFilePath, outputs: page.outputRecords }
}
