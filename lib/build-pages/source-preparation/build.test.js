...
async function fixture (t, files) {
  ...
  // Discovery/bootstrap happens before installing process-local work counters.
  const { siteData } = await site.build()
  return {
    ...site,
    siteData,
...
test('fresh workers return a small source-only delta and rerender cached Markdown selected by global-data changes', options, async t => {
  ...
  const site = await fixture(t, {
    ...
    'b/page.md': article('Beta'),
    'c/page.md': article('Untouched', 'Large unchanged body. '.repeat(1024)),
  ...
  for (const title of ['Second selection', 'Third selection']) {
    ...
    assert.ok(serialize(update).byteLength < serialize(initialUpdate).byteLength / 10, 'unchanged bodies are not echoed across the worker boundary')
    assert.deepEqual(result.report.globalDataBaseline?.state, {
    ...
    assert.equal(next.get(a), previous.get(a), 'the accepted delta preserves unchanged prepared Markdown')
