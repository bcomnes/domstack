...
test('hits isolate vars without reloading or upserting; misses snapshot before exposure', async t => {
  const p = page()
  const loader = t.mock.fn(async () => prepared())
  const first = new MarkdownPreparationCache({ pages: [p] }, loader)
  ...
  const next = new MarkdownPreparationCache({ pages: [p], previous, changes: delta }, loader)
  ...
  assert.equal(loader.mock.callCount(), 1, 'only the initial miss prepares source')
  assert.equal(hit.markdownContent, '# Original')
...
test('concurrent preparation loads once and gives each consumer isolated vars', async t => {
  ...
  const cache = new MarkdownPreparationCache({ pages: [p] }, loader)
  const pending = Promise.all([cache.prepare(p), cache.prepare(p)])
  assert.equal(loader.mock.callCount(), 1, 'concurrent requests share the pending load')
  ready.resolve(undefined)
  const results = await pending
  assert.equal(loader.mock.callCount(), 1)
  assert.equal(cache.getUpdate().upserts.size, 1)
  assert.notEqual(results[0]?.vars['nested'], results[1]?.vars['nested'])
