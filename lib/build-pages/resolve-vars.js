 ...
 * @param {PageData<any, any, any, any>[]} params.pages - Initialized source-backed PageData array.
 * @param {GlobalDataFunctionParams} [params.context] - State context prepared by the page phase.
 * @returns {Promise<object>}
 */
export async function resolveGlobalData ({ globalDataPath, pages, context }) {
  ...
  if (isFunction(maybeGlobalData)) {
    const result = await maybeGlobalData(context ?? createGlobalDataState({ pages }).context)
