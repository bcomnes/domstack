...
export function resolveDataDeps (value, label) {
  ...
  if (!Array.isArray(value)) {
    throw new TypeError(`${label} dataDeps must be an array of strings`)
  }
  ...
  for (const key of value) {
    if (typeof key !== 'string' || key.length === 0) {
      throw new TypeError(`${label} dataDeps must contain non-empty strings`)
    }
 ...
 * @param {string} label
 * @returns {{ vars: T, dataDeps: string[] }}
 */
