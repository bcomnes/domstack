...
describe('WatchDependencyTracker', () => {
  ...
  test('prunes observations for generated pages that no longer exist', async () => {
    const tracker = new WatchDependencyTracker(null, { fullBuild: true })
    const removedPage = tracker.trackPageData(/** @type {any} */ ({
      pageInfo: {
        generated: {
          pagesFile: {
            pagesFile: { filepath: '/site/archive.pages.js' },
          },
        },
        outputRelname: 'archive/removed/index.html',
        pageFile: { filepath: '/site/archive.pages.js' },
      },
    }))
    const observeRemovedPage = async () => removedPage.pageInfo.outputRelname

    await tracker.runWithConsumer(
      'page',
      'archive/removed/index.html',
      observeRemovedPage,
      { ownerPath: '/site/archive.pages.js' }
    )
    await tracker.runWithConsumer('template', 'archive.html', observeRemovedPage)

    tracker.pruneGeneratedPages(new Set())

    assert.equal(tracker.state.consumers['page\0archive/removed/index.html'], undefined)
    assert.equal(tracker.state.pageFingerprints['archive/removed/index.html'], undefined)
    assert.deepEqual(tracker.state.consumers['template\0archive.html']?.pages, {})
  })
})
