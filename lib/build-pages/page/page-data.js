...
import { resolvePageOutputProvider } from '../outputs/resolve-page-output-provider.js'
...
export class PageData {
  ...
  /** @type {PageFunction<T, U, D> | null} Renderer prepared for this page's build lifetime. */ #render = null
  ...
  get vars () {
    ...
    try {
      return this.#vars.getCached(this)
    } catch (err) {
      throw new Error(
  ...
  /**
   * Prepare the renderer and resolve vars, layout chain, and assets once before rendering.
   * HTML/Markdown source is captured here; render inputs are supplied on each call.
  ...
  async init ({ layouts }) {
    ...
    const { vars: resolvedPageVars, exports: companionExports } = await resolvePageCompanion(pageVars?.filepath)
    ...
    const { provider, warning } = resolvePageOutputProvider(pageInfo, built.pageOutputs, companionExports)
    this.#pageOutputs = provider
    ...
    this.pageVars = /** @type {Partial<T>} */ (pageResolution.vars)
    ...
    this.dataDeps = this.#subscriptions.collectDependencies()
    ...
    /** @type {object} */
    const finalVars = this.#vars.mergeUncached(this)
    ...
    this.#render = /** @type {PageFunction<T, U, D>} */ (built.pageLayout)
  ...
  async renderInnerPage () {
    ...
    if (!this.#initialized || !render) throw new Error('Must be initialized before rendering inner pages')
    const { pageInfo, styles, scripts, vars, data, workers } = this
