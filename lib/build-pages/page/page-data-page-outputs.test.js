...
test('initialization preserves companion vars, postVars, builder, then hook error ordering', async t => {
  for (const [companion, expected] of [
    ['export default () => null; export const postVars = true', /Var function must resolve to a plain object/],
    ['export const postVars = true; export const pageOutputs = null', /postVars is no longer supported/],
    ['export const pageOutputs = null', /builder failed/],
  ]) {
    assert.ok(typeof companion === 'string')
    assert.ok(expected instanceof RegExp)
    const { pd, layouts } = await fixture(t, {
      module: "throw new Error('builder failed')",
      companion,
    })
    await assert.rejects(pd.init({ layouts }), expected)
  }
})

test('provider selection observes companion export changes made by the page builder', async t => {
  const { pd, layouts } = await fixture(t, {
    module: "import { replaceHook } from './page.vars.mjs'; replaceHook(); export default () => ''",
    companion: `export let pageOutputs = null
      export function replaceHook () { pageOutputs = () => ({ outputName: 'live.txt', content: 'live' }) }`,
  })
  await pd.init({ layouts })
  assert.equal((await Array.fromAsync(pd.collectPageOutputs()))[0]?.content, 'live')
})
