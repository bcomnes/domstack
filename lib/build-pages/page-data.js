 ...
 * @import { AdditionalOutputsFunction, AdditionalOutputProvenance, CollectedAdditionalOutput } from './additional-outputs.js'
 * @import { JsPageBuilderResult } from './page-builders/js/index.js'
 */
...
import { createAdditionalOutputsPage, normalizeAdditionalOutputs, validateAdditionalOutputsHook } from './additional-outputs.js'
 ...
 * @returns {Promise<{ render: InternalLayoutFunction<T, U, V, D>, vars: Partial<T>, parentLayout: string | undefined, additionalOutputs: AdditionalOutputsFunction<T, D> | undefined, source: string }>} The resolved layout module exports.
 */
export async function resolveLayout (layoutPath) {
  const { default: layout, vars, parentLayout, additionalOutputs } = await import(layoutPath)
  ...
  return {
    ...
    parentLayout,
    source: layoutPath,
    additionalOutputs: validateAdditionalOutputsHook(additionalOutputs, layoutPath),
 ...
 * @property {AdditionalOutputsFunction<T, D> | undefined} [additionalOutputs] - Explicit output-phase hook.
...
export class PageData {
  ...
  /** @type {{ hook: AdditionalOutputsFunction<T, any>, provenance: AdditionalOutputProvenance } | undefined} */ #pageAdditionalOutputs
  ...
  async init ({ layouts }) {
    ...
    if (!pageInfo.generated) {
      const moduleHook = type === 'js' ? /** @type {JsPageBuilderResult<T, U>} */ (built).additionalOutputs : undefined
      const companionHook = pageVars?.filepath
        ? validateAdditionalOutputsHook((await import(pageVars.filepath)).additionalOutputs, pageVars.filepath)
        : undefined
      if (moduleHook && companionHook) {
        throw new Error(`Page "${pageInfo.pageFile.filepath}" and companion "${pageVars?.filepath}" both export additionalOutputs; only one page-level provider is allowed`)
      }
      const hook = moduleHook ?? companionHook
      if (hook) {
        this.#pageAdditionalOutputs = {
          hook,
          provenance: {
            kind: moduleHook ? 'page' : 'companion',
            source: moduleHook ? pageInfo.pageFile.filepath : /** @type {string} */ (pageVars?.filepath),
          },
        }
      }
    }
  ...
  /**
   * Run hooks only when explicitly requested by the output phase.
   * Layouts run outermost first, followed by the single page-level provider.
   * @returns {Promise<CollectedAdditionalOutput[]>}
   */
  async collectAdditionalOutputs () {
    if (!this.#initialized) throw new Error('Must be initialized before collecting additionalOutputs')
    ...
    const page = createAdditionalOutputsPage(sourceInfo, async () => {
    ...
    /**
     * @param {AdditionalOutputsFunction<T, any>} hook
     * @param {AdditionalOutputProvenance} provenance
     * @param {() => object} getData
     */
    const collect = async (hook, provenance, getData) => {
      try {
        outputs.push(...await normalizeAdditionalOutputs(hook({ page, vars: this.vars, data: getData() }), provenance))
      } catch (cause) {
        throw new Error(`additionalOutputs for page "${this.pageInfo.pageFile.relname}" from ${provenance.kind} "${provenance.source}" failed: ${cause instanceof Error ? cause.message : String(cause)}`, { cause })
      }
    }
    for (const layout of this.layoutChain) {
      const source = layout.source ?? layout.name
      const hook = validateAdditionalOutputsHook(layout.additionalOutputs, source)
      if (!hook) continue
      await collect(hook, { kind: 'layout', source, layoutName: layout.name }, () => {
    ...
    if (this.#pageAdditionalOutputs) {
      const { hook, provenance } = this.#pageAdditionalOutputs
      await collect(hook, provenance, () => this.data)
    }
    return outputs
  }
