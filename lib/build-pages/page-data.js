...
export class PageData {
  ...
  async * collectAdditionalOutputs () {
    ...
    const collect = async function * (hook, provenance, getData) {
      try {
        yield * normalizeAdditionalOutputs(hook({ page, vars: pageData.vars, data: getData() }), provenance)
      } catch (cause) {
        throw new Error(`additionalOutputs for page "${pageData.pageInfo.pageFile.relname}" from ${provenance.kind} "${provenance.source}" failed: ${cause instanceof Error ? cause.message : String(cause)}`, { cause })
      }
    }
    for (const layout of this.layoutChain) {
