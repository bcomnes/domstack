 ...
 * @returns {Promise<{ render: InternalLayoutFunction<T, U, V, D>, vars: Partial<T>, parentLayout: string | undefined, pageOutputs: PageOutputsFunction<T, D> | undefined, source: string }>} The resolved layout module exports.
 */
export async function resolveLayout (layoutPath) {
  const { default: layout, vars, parentLayout, pageOutputs } = await import(layoutPath)
...
export class PageData {
  ...
  get vars () {
    ...
    if (
      ...
      this.#varsCacheSources.length === sources.length &&
    ...
    try {
      this.#varsCache = /** @type {T} */ (Object.freeze(
        sources.reduce((merged, vars) => ({ ...merged, ...vars }), {})
      ))
  ...
  async init ({ layouts }) {
    ...
    /** @type {object} */
    const finalVars = this.#varSources().reduce((merged, vars) => ({ ...merged, ...vars }), {})
  ...
  async * collectPageOutputs () {
    ...
    const collect = async function * (hook, provenance, getData) {
      try {
        ...
        throw new Error(`pageOutputs for page "${pageData.pageInfo.pageFile.relname}" from ${provenance.kind} "${provenance.source}" failed: ${cause instanceof Error ? cause.message : String(cause)}`, { cause })
      }
    }
    for (const layout of this.layoutChain) {
