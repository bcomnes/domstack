...
import { createSubscribedData, extractDataDependencies } from './watch-dependencies.js'
...
/**
  ...
  * @template [V=string] V - The return type of the layout function (defaults to string)
  * @template {object} [D=Record<string, unknown>] - Declared global data.
  * @typedef {LayoutFunction<T, U, V, D> | AsyncLayoutFunction<T, U, V, D>} InternalLayoutFunction
  */
...
export class PageData {
  ...
  /** @type {ResolvedLayout<T, U, V, D> | null | undefined} */ layout
  ...
  /** @type {string[]} Union of the page and entire layout chain, for output invalidation. */ dataDependencies = []
  /** @type {string[]} */ #pageDataDependencies = []
  /** @type {Map<string, { keys: string[], data: D }>} */ #layoutSubscriptions = new Map()
  /** @type {string[]} */ styles = []
  /** @type {string[]} */ scripts = []
   ...
   */
  get data () {
    if (!this.#dataReady && this.#pageDataDependencies.length > 0) {
      throw new Error(`Global data is not available while resolving global.data for page "${this.pageInfo.pageFile.relname}"`)
    }
    return this.#data
  }
  ...
  setGlobalData (globalData) {
    this.#data = /** @type {D} */ (createSubscribedData(
      globalData,
      this.#pageDataDependencies,
      `Page "${this.pageInfo.pageFile.relname}"`
    ))
    for (const [name, subscription] of this.#layoutSubscriptions) {
      subscription.data = /** @type {D} */ (createSubscribedData(globalData, subscription.keys, `Layout "${name}"`))
    }
    this.#dataReady = true
  }
  ...
  async init ({ layouts }) {
    ...
    const pageResolution = extractDataDependencies(resolvedPageVars, `Page vars "${pageInfo.pageFile.relname}"`)
    const builderResolution = extractDataDependencies(builderVars, `Page "${pageInfo.pageFile.relname}"`)
    this.pageVars = pageResolution.vars
    this.builderVars = /** @type {Partial<T>} */ (builderResolution.vars)
    this.#pageDataDependencies = [...new Set([...pageResolution.dataDependencies, ...builderResolution.dataDependencies])].sort()
    const dependencies = new Set(this.#pageDataDependencies)
    for (const layout of this.layoutChain) {
      const resolution = extractDataDependencies(layout.vars, `Layout "${layout.name}"`)
      this.#layoutSubscriptions.set(layout.name, { keys: resolution.dataDependencies, data: /** @type {D} */ (Object.freeze({})) })
      Object.assign(this.layoutVars, resolution.vars)
      for (const key of resolution.dataDependencies) dependencies.add(key)
    ...
    this.dataDependencies = [...dependencies].sort()
  ...
  async renderFullPage () {
    ...
    if (!this.#dataReady && this.dataDependencies.length > 0) {
      throw new Error(`Global data is not available while resolving global.data for page "${this.pageInfo.pageFile.relname}" or its layouts`)
    }
    ...
    for (const currentLayout of layoutChain.toReversed()) {
      rendered = await currentLayout.render({
        ...
        scripts,
        page: pageInfo,
        data: /** @type {D} */ (this.#layoutSubscriptions.get(currentLayout.name)?.data),
        children: rendered,
