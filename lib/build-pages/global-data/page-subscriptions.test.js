...
test('keeps page and layout access narrower than the invalidation union', () => {
  ...
  assert.deepEqual(subscriptions.collectDependencies(), ['author', 'navigation', 'posts', 'shared'])
...
test('guards only declared data before binding and preserves readiness error metadata', () => {
  ...
  assert.throws(() => subscriptions.assertReady(subscriptions.collectDependencies(), pageInfo), /Global data is not available/)
  subscriptions.bind({ navigation: [] }, pageInfo)
  assert.doesNotThrow(() => subscriptions.assertReady(subscriptions.collectDependencies(), pageInfo))
})
