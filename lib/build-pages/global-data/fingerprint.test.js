...
const primitiveCases = [
  ...
  { name: 'true string', value: 'true', json: '"true"' },
  { name: 'false string', value: 'false', json: '"false"' },
  { name: 'zero string', value: '0', json: '"0"' },
  { name: 'one string', value: '1', json: '"1"' },
  { name: 'quotes and slashes', value: '"\\/', json: '"\\"\\\\/"' },
  { name: 'short control escapes', value: '\b\f\n\r\t', json: '"\\b\\f\\n\\r\\t"' },
  { name: 'other control escapes', value: '\u0000\u0001\u000b\u001f', json: '"\\u0000\\u0001\\u000b\\u001f"' },
  { name: 'newline', value: '\n', json: '"\\n"' },
  { name: 'literal newline escape', value: '\\n', json: '"\\\\n"' },
  { name: 'literal surrogate escape', value: '\\ud800', json: '"\\\\ud800"' },
...
function assertLegacyEquivalent (factory, label) {
  ...
  return { hash: previous, events: optimized.events }
}
...
// These tests share JSON spies and temporary prototype hooks.
...
describe('global-data fingerprint compatibility', { concurrency: false }, () => {
  ...
  test('compares incremental builds against a legacy fingerprint snapshot', () => {
    const unchanged = { primitive: 'same', graph: { nested: [1, true] } }
    ...
    assert.deepEqual([...next.updateGlobalDataFingerprints(data, previousState.globalDataFingerprints)], [
      'removed', 'changed', 'typeChanged', 'becameOpaque', 'becameSupported', 'opaque', 'added',
    ])
  ...
  test('keeps unsupported primitives opaque on every build, including inside objects and arrays', async t => {
    for (const [name, value] of Object.entries(opaquePrimitives)) {
      await t.test(name, () => {
        ...
        for (let build = 0; build < 2; build++) {
          ...
          assert.deepEqual([...tracker.updateGlobalDataFingerprints(data, previous.globalDataFingerprints)], Object.keys(data), `build ${build}: invalidation`)
          assert.deepEqual(Object.values(tracker.state.globalDataFingerprints), [null, null, null], `build ${build}: opaque hashes`)
        }
      })
    }
  })
  ...
  test('retains inherited Array.prototype.toJSON behavior and conservative serialization failures', () => {
    ...
    const calls = []
    const replacements = { object: { z: 'converted', a: true }, string: 'converted', negativeZero: -0, nan: NaN, undefined, bigint: 1n }
    ...
    try {
      for (const [name, replacement] of Object.entries(replacements)) {
        ...
        const tracker = new WatchDependencyTracker(null, { fullBuild: true })
        tracker.updateGlobalDataFingerprints({ value }, null)
        assert.equal(tracker.state.globalDataFingerprints['value'], expected, `${name}: hash`)
        assert.equal(calls.length, 1, `${name}: hook count`)
        assert.equal(calls[0]?.receiver, value, `${name}: receiver`)
        assert.equal(calls[0]?.key, '', `${name}: root key`)
        assert.deepEqual([...tracker.updateGlobalDataFingerprints({ value }, tracker.state.globalDataFingerprints)], expected === null ? ['value'] : [], `${name}: invalidation`)
      }
      Object.defineProperty(Array.prototype, 'toJSON', {
        configurable: true,
  ...
  test('matches seeded nested graphs', () => {
    for (let seed = 0; seed < 80; seed++) {
      assertLegacyEquivalent(() => ({ value: seededGraph(seed), events: [] }), `seed ${seed}`)
    }
  })

  test('preserves stable Unicode collation ties', () => {
    ...
    for (const [name, value] of Object.entries({ forward, reverse, aliases: [forward, reverse, forward] })) {
      assertLegacyEquivalent(() => ({ value, events: [] }), `${name}: Unicode ties and numeric keys`)
    }
  })

  test('rejects nested lossy shapes without running accessors or own JSON hooks', async t => {
    ...
    for (const [name, make] of Object.entries(factories)) {
      await t.test(name, () => {
        const actual = assertLegacyEquivalent(() => {
          /** @type {string[]} */
          const events = []
          return { value: { safe: [1, { a: 'first' }], bad: [make(events)] }, events }
        }, name)
        assert.equal(actual.hash, null, 'lossy shapes must be opaque')
        assert.deepEqual(actual.events, [], 'validation must not invoke user code')
      })
    }
  })
  ...
  test('raw JSON preserves legacy numeric canonicalization and string/number separation', t => {
    ...
    for (const { text, canonical } of cases) {
      assert.equal(legacyFingerprint(rawJSON(text)), sha256(canonical), text)
  ...
  test('boxed primitives with null or Object.prototype retain legacy internal-slot behavior', () => {
    ...
    for (const prototype of [null, Object.prototype]) {
      for (const { name, value } of cases) {
        const label = `${name}, ${prototype === null ? 'null' : 'Object.prototype'}`
        const makeBox = () => Object.setPrototypeOf(Object(value), prototype)
        const expected = legacyFingerprint(makeBox())
        if (typeof value === 'boolean') assert.equal(expected, sha256(String(value)), label)
        if (typeof value === 'number') assert.equal(expected, prototype === null ? null : sha256('null'), label)
        if (typeof value === 'bigint' || typeof value === 'string') assert.equal(expected, null, label)
        if (typeof value === 'symbol') assert.equal(expected, sha256('{}'), label)
        for (const [location, wrap] of Object.entries(wrappers)) {
          assertLegacyEquivalent(() => ({ value: wrap(makeBox()), events: [] }), `${label}, ${location}`)
        }
      }
      const tracker = new WatchDependencyTracker(null, { fullBuild: true })
      const boxedTrue = Object.setPrototypeOf(Object(true), prototype)
      const boxedFalse = Object.setPrototypeOf(Object(false), prototype)
      const label = prototype === null ? 'null' : 'Object.prototype'
      tracker.updateGlobalDataFingerprints({ value: boxedTrue }, null)
      assert.deepEqual([...tracker.updateGlobalDataFingerprints({ value: boxedFalse }, tracker.state.globalDataFingerprints)], ['value'], `${label}: true -> false`)
      assert.equal(tracker.state.globalDataFingerprints['value'], sha256('false'), label)
      assert.deepEqual([...tracker.updateGlobalDataFingerprints({ value: boxedFalse }, tracker.state.globalDataFingerprints)], [], `${label}: repeated false`)
    }
  })

  test('retains array subclass and custom prototype semantics', () => {
    /** @type {Record<string, (events: string[]) => unknown>} */
    const factories = {
    ...
    for (const prototype of [Object.prototype, Array.prototype]) {
      ...
      try {
        for (const mode of ['identity', 'replacement', 'noncallable', 'undefined', 'throw']) {
          const label = `${prototype === Object.prototype ? 'Object' : 'Array'}.prototype: ${mode}`
          ...
          assert.ok(expectedEvents.length > 0, label)
          events = []
          const tracker = new WatchDependencyTracker(null, { fullBuild: true })
          tracker.updateGlobalDataFingerprints({ value: makeValue() }, null)
          assert.deepEqual(events, expectedEvents, `${label}: hooks must not run twice during fallback`)
  ...
  test('top-level, nested, and layered proxies match legacy traps without speculative reads', async t => {
    const cases = [
      { name: 'top-level object', array: false, layers: 1, nested: false },
      { name: 'nested object', array: false, layers: 1, nested: true },
      { name: 'nested layered object', array: false, layers: 2, nested: true },
      { name: 'nested array', array: true, layers: 1, nested: true },
      { name: 'nested layered array', array: true, layers: 2, nested: true },
    ]
    for (const { name, array, layers, nested } of cases) {
      await t.test(name, () => {
        assertLegacyEquivalent(() => {
          /** @type {string[]} */
          const events = []
          ...
          return { value: nested ? { first: { safe: [1, 2] }, nested: [{ value }], alias: value } : value, events }
        }, name)
      })
    }
  })
  ...
  test('deep graphs match legacy results rather than imposing a supported-depth limit', () => {
    for (const depth of [127, 128, 129, 256, 1024, 16384]) {
      for (const array of [false, true]) {
        ...
        const expected = legacyFingerprint(value)
        const tracker = new WatchDependencyTracker(null, { fullBuild: true })
        const changed = tracker.updateGlobalDataFingerprints({ value }, { value: expected })
        const label = `${array ? 'array' : 'object'} depth ${depth}`
        assert.equal(tracker.state.globalDataFingerprints['value'], expected, label)
        assert.deepEqual([...changed], expected === null ? ['value'] : [], `${label}: invalidation`)
      }
    }
  })
