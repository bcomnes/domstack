...
import { createMdResolver } from './page-builders/md/create-md-resolver.js'
import { MarkdownPreparationCache } from './source-preparation/markdown-cache.js'
...
export async function buildPagesDirect (_src, dest, siteData, opts) {
  ...
  const markdownPreparation = opts?.trackWatchDependencies
    ? new MarkdownPreparationCache({
      pages: siteData.pages,
      previous: opts.previousMarkdownPreparation,
      changes: opts.globalDataInputChanges,
    })
    : null
  ...
  const builderOptions = {
    ...
    getMarkdownRenderer: createMdResolver(),
    prepareMarkdown: markdownPreparation ? page => markdownPreparation.prepare(page) : undefined,
  }
   ...
   */
  const recordPageOutputs = (page, pageFilePath, outputs) => {
  ...
  const writePage = async (page) => {
    try {
      ...
      recordPageOutputs(page, buildResult.pageFilePath, buildResult.outputs)
      return true
    } catch (err) {
      ...
      if (page.outputRecords.length > 0) {
        recordPageOutputs(page, join(dest, page.pageInfo.outputRelname), page.outputRecords)
      }
  ...
  if (result.errors.length === 0) {
    if (markdownPreparation) result.report.markdownPreparationUpdate = markdownPreparation.getUpdate()
    if (opts?.trackWatchDependencies && globalDataState) result.report.globalDataBaseline = globalDataState.getBaseline()
  }
  return result
}
