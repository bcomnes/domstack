...
test('settings changes reload bundleRoots and reuse the module for identical contents', async t => {
  ...
  let originalCalls = 0
  for (const [contents, expected] of /** @type {const} */ ([[first, 'admin'], [second, 'other'], [second, 'other'], [first, 'admin']])) {
    await writeFile(path, contents)
    const watch = await buildEsbuildWatch(src, dest, siteData, {})
    try {
      const metafile = JSON.stringify(watch.buildResults.metafile)
      assert.equal(metafile.includes('/admin/chunks/js/'), expected === 'admin')
      if (contents === first) originalCalls++
      assert.equal(original.calls.length, originalCalls, 'initial and reverted content share the ordinary import state')
    } finally {
      await watch.context.dispose()
    }
  }
  ...
  assert.equal(original.calls.length, originalCalls + 1, 'production reuses the same first-content module')
...
test('CommonJS settings reload their named bundleRoots export', async t => {
  const { src, dest } = await createFixture(t, '')
  await mkdir(dest)
  await rm(join(src, 'esbuild.settings.js'))
  ...
  for (const root of ['admin', 'other', 'admin']) {
    ...
    const result = await buildEsbuild(src, dest, await identifyPages(src), {})
    assert.deepEqual(result.errors, [])
    assert.deepEqual(result.report.builds?.map(build => build.bundleRoot), [null, root])
    if (root === 'admin') originalCalls++
    assert.equal(original.default.calls.length, originalCalls)
  }
  assert.equal(original.default.calls.length, 2)
})
